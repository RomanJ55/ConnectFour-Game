from tkinter import *
from constants import MARGIN, CELL_WIDTH, LIGHTRED, LIGHTBLUE
...
class Connect_Four_UI(Frame):
    ...
    def __draw_pieces(self):
        ...
        self.canvas.create_text(
            425, 725, fill=self.connectfour.get_turn(), tag="turncol", text=f"{turn.upper()} Turn", font="comicsans 30")
        ...
        # remove for 2 player mode
        else:
            if turn == "blue":
                self.ai_move()

    def __draw_possible_moves(self):
        moves = self.connectfour.possible_moves(self.connectfour.board)
        turn = self.connectfour.get_turn()
        for move in moves:
            x0 = 55 + move[0]*CELL_WIDTH
            y0 = 15
            x1 = 55 + move[0]*CELL_WIDTH
            y1 = MARGIN//2 + 10
            self.canvas_top.create_line(
                x0, y0, x1, y1, tag="arrows", arrow="last", fill=turn, width=5)

            move_y, move_x = move[0], move[1]
            x0 = MARGIN + move_y*CELL_WIDTH
            y0 = MARGIN + move_x*CELL_WIDTH
            x1 = MARGIN + (move_y+1)*CELL_WIDTH
            y1 = MARGIN + (move_x+1)*CELL_WIDTH
            showcolor = LIGHTRED
            if turn == "blue":
                showcolor = LIGHTBLUE
            self.canvas.create_oval(
                x0+6, y0+6, x1-6, y1-6, fill=showcolor)
    ...
    def game_end(self):
        if self.connectfour.is_winner(self.connectfour.board):
            x, y = self.connectfour.is_winner(self.connectfour.board)
            return x, y
    ...
    def ai_move(self):
        possible = self.connectfour.possible_moves(self.connectfour.board)
        move = self.connectfour.get_best_move(possible)
        self.__add_piece(move[1], move[0])
        self.__draw_pieces()
